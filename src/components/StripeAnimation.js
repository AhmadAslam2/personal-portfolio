    ...
    >
      {isActive ? (
        ...
        <Image
          ...
          style={{
            filter:
              "brightness(0) saturate(100%) invert(64%) sepia(5%) saturate(0%) hue-rotate(0deg) brightness(95%) contrast(90%)",
          }}
          unoptimized
        />
      )}
  ...
  useEffect(() => {
    const calculateDimensions = () => {
      ...
      if (windowWidth >= 768) {
        ...
        const squareSizeByHeight = Math.floor(
          (availableHeight - (maxRows - 1) * gap) / maxRows
        );
        ...
        squareSize = Math.max(
          30,
          Math.min(squareSizeByWidth, squareSizeByHeight)
        );
      }
  ...
  useEffect(() => {
    ...
    const activateNextSequence = () => {
      ...
      sequenceIndexRef.current =
        (sequenceIndexRef.current + 1) % activationSequence.length;
    };
  ...
  return (
    ...
    >
      ...
      >
        {gridItems.map((item) => {
          const isActive =
            activeIndices.includes(item.id) || hoveredId === item.id;
          const { skill } = item;
